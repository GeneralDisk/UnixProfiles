...
tester()
{
        ...
        if [[ $test_counter -eq 0 ]]
                then
                        echo "zero!"
                        test_counter=$((test_counter - 1))
                else
                        if [[ $test_counter -gt -1 ]]
                        then
                                test_counter=$((test_counter - 1))
                        fi
        fi
...
upload_atom_scripts()
{
        ...
        #        done
        #else
        #        echo "you must pass args with this command."
        #        help
        #        return 1
        #fi
        ...
        if [ -z "$1" ]
        then
                echo "Error: you must set a target testbed to build to (ex: js92-5)"
                return 1
        else
                TARGET_TESTBED="$1"
        fi

        #TARGET_ATOM_PROJECT="disable_remote_rpc"
        TARGET_ATOM_PROJECT="simple_load_fw_tables"

        echo "Building and uploading $TARGET_ATOM_PROJECT scripts to $TARGET_TESTBED for testing"

        # Step 1 build
        BUILD_CMD="make dev name=$TARGET_ATOM_PROJECT"
        ATOM_PROJECT_DIR="/home/mkali/work/cdu-atom-scripts"

        echo "1:1 - Going to Atom directory to build $TARGET_ATOM_PROJECT"
        cd $ATOM_PROJECT_DIR

        echo "1:2 - Running '$BUILD_CMD'"
        $BUILD_CMD

        # Step 2 upload to both controllers.  Start with CT0
        BUILD_DIR="$ATOM_PROJECT_DIR/build/$TARGET_ATOM_PROJECT"
        DEST_DIR="~/atom_test"

        TARGET_CT="$TARGET_TESTBED-ct0"
        echo "2:1 - Uploading new artifacts to $TARGET_CT"

        SCP_CMD1="scp $BUILD_DIR/atom_bundle.sh root@$TARGET_CT:$DEST_DIR/atom_bundle.sh"
        SCP_CMD2="scp $BUILD_DIR/upgrade_checks root@$TARGET_CT:$DEST_DIR/upgrade_checks"
        SCP_CMD3="scp $BUILD_DIR/upgrade_checks-stg.tar.gz root@$TARGET_CT:$DEST_DIR/upgrade_checks-stg.tar.gz"
        SCP_CMD3="scp $BUILD_DIR/simple_mitigation_bundle.zip root@$TARGET_CT:$DEST_DIR/simple_mitigation_bundle.zip"

        $SCP_CMD1
        $SCP_CMD2
        $SCP_CMD3

        TARGET_CT="$TARGET_TESTBED-ct1"
        echo "2:2 - Uploading new artifacts to $TARGET_CT"
...
run()
{
        ...
        $COMMAND 2>&1 | {
                while IFS= read -r line
                do
                        # decrement the lines until we get to -1
                        if [[ $COUNTER -gt -1 ]]
                        then
                                COUNTER=$((COUNTER- 1))
                        fi
                        if [[ $COUNTER -eq 0 ]]
                                then
                                        ...
                                        #echo "Logfile we care about = $line"
                                        LINE_AR=( $line )
                                        TEST_LOG=''
                                        TEST_LOG=${LINE_AR[-1]}
                        ...
                        do
                                ...
                                if [[ "$word" =~ "FAILED" ]]
                                then
                                        # We print the total path two lines after the failed line,
                                        # so set a counter and count down...
                                        COUNTER=2
                                        IS_FAIL_LINE="true"
                                        TEST_CASE=''
                                        OPEN_LOG_FILE="true"
                                        ...
                                        #TEST_CASE="${line##* }"
                                        #TEST_CASE=${TEST_CASE%^*}
                                        # Keep track of the test cases, it will make things easier to
                                        # navigate later on
        ...
        then
                ...
                then
                        ...
                        echo "Opening log file for $TEST_CASE : $TEST_LOG"
                        vi $TEST_LOG
                        ...
                        do
                                ...
                                TEST_LOG="${TEST_LOG_FILES[choice]}"
                                ...
                                if [ -z TEST_LOG ]
                                then
                                        echo "ERROR: $TEST_LOG does not exist for failed testcase $TEST_CASE!"
                                        continue
                                fi
                                #LOG_FILE="$PLAN/${TEST_CASE}__${TASK}__failed.stdout"
                                echo "Opening log file: $TEST_LOG"
                                vi $TEST_LOG
                        done
                fi
        fi
